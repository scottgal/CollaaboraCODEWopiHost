...
public class WopiFileSystemProvider : IWopiStorageProvider
{
    ...
    /// <summary>
    ///     Creates a new instance of the <see cref="WopiFileSystemProvider" /> based on the provided hosting environment and
    ///     configuration.
    /// </summary>
    ...
    public WopiFileSystemProvider(IHostEnvironment env, IConfiguration configuration)
    {
        ...
        FileSystemProviderOptions = configuration.GetSection(WopiConfigurationSections.STORAGE_OPTIONS)
            .Get<WopiFileSystemProviderOptions>(); //TODO: rework
    }
    ...
    private string WopiAbsolutePath => Path.IsPathRooted(WopiRootPath)
        ? WopiRootPath
        : Path.Combine(HostEnvironment.ContentRootPath, WopiRootPath);
    ...
    public List<IWopiFile> GetWopiFiles(string identifier = "")
    {
        ...
        foreach (var path in
                 Directory.GetFiles(Path.Combine(WopiAbsolutePath, folderPath))) //TODO Directory.Enumerate...
        {
