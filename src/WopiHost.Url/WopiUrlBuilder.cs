...
public partial class WopiUrlBuilder
{
    ...
    /// <param name="urlSettings">
    ///     Additional URL settings (if not specified, defaults passed to the class constructor will be
    ///     used).
    /// </param>
    /// <returns></returns>
    public async Task<string> GetFileUrlAsync(string extension, Uri wopiFileUrl, WopiActionEnum action,
        WopiUrlSettings urlSettings = null)
    {
        ...
        if (!string.IsNullOrEmpty(template))
        {
            ...
            var url = UrlParamRegex().Replace(template,
                m => ResolveOptionalParameter(m.Groups["name"].Value, m.Groups["value"].Value, combinedUrlSettings));
            url = url.TrimEnd('&');
    ...
    {
        if (urlSettings.TryGetValue(value, out var param)) return name + "=" + Uri.EscapeDataString(param) + "&";
        return null;
    }
}